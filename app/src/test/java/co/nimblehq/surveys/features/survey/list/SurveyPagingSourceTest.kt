...
class SurveyPagingSourceTest {
    ...
    @Test
    fun `When loads a single page successfully, it returns survey list `() = runTest {
        val surveyList = List(10) { index ->
            SurveyModel(
                id = "$index",
    ...
    @Test
    fun `When loads consecutive pages successfully, it returns survey list by page`() = runTest {
        val surveyList1 = List(10) { index ->
            SurveyModel(
                id = "$index",
                title = "Survey title $index",
                description = "Survey description $index",
        ...
        val result1 = with(pager) {
            refresh()
        } as PagingSource.LoadResult.Page

        result1.data shouldBe surveyList1

        val result2 = with(pager) {
            append()
        } as PagingSource.LoadResult.Page
    ...
    @Test
    fun `When refresh a page is fail, it returns a page error `() = runTest {
        val error = Throwable()
        coEvery {
            val params = GetSurveyListUseCase.Params(1, SurveyPageConfig.PAGE_SIZE)
            getSurveyListUseCase(params)
