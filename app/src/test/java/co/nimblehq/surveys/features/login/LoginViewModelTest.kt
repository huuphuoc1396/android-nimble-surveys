...
class LoginViewModelTest : ViewModelTest() {
    ...
    @Test
    fun `When email and password valid, it enables login button`() =
        runTest {
    ...
    @Test
    fun `When email and password invalid, it disables login button`() =
        runTest {
    ...
    @Test
    fun `When email changed, it updates email in the UiState`() =
        runTest {
            loginViewModel.onEmailChanged("tester@mail.com")
            loginViewModel.uiStateFlow.test {
    ...
    @Test
    fun `When password changed, it updates password in the UiState`() =
        runTest {
            loginViewModel.onPasswordChanged("12345678")
            loginViewModel.uiStateFlow.test {
    ...
    @Test
    fun `When login click, it logins successfully then sends GoToHome event `() =
        runTest {
            loginViewModel.onEmailChanged("tester@mail.com")
            loginViewModel.onPasswordChanged("12345678")
            coEvery {
    ...
    @Test
    fun `When login click, it logins fail then throw error`() =
        runTest {
            val error = Throwable()
            loginViewModel.onEmailChanged("tester@mail.com")
            loginViewModel.onPasswordChanged("12345678")
    ...
    @Test
    fun `When forgot password click, it sends GoToForgotPassword event`() =
        runTest {
