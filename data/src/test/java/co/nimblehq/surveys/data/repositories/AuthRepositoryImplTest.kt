...
class AuthRepositoryImplTest {
    ...
    @Test
    fun `When login is successful, it saves tokens to storage and returns true`() = runTest {
        val tokenType = "Bear"
    ...
    @Test
    fun `When login responses data null, it returns false`() = runTest {
        val loginRequest = LoginRequest(
            email = "tester@mail.com",
            password = "1234",
            grantType = "password",
    ...
    @Test
    fun `When logout is fail, it throws error`() = runTest {
        val error = Exception()
        val accessToken = "92yE8gBkoh1bK7I1p0Dd5vV0xVVUT5uIcttj2LQbcFiCTjGFdz50fgQ7z0MwwkZw"
        val logoutRequest = LogoutRequest(
            token = accessToken,
    ...
    @Test
    fun `When user didn't login, it emits false`() = runTest {
